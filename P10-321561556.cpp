...
void saveFrame(void) //tecla L
{
	...
	KeyFrame[FrameIndex].giroAlaIzq = giroAlaIzq;
	KeyFrame[FrameIndex].giroAlaDer = giroAlaDer;
...
int main()
{
	...
	*/
	...
	std::ifstream frames("newFrames.txt");
	std::string frame;
	...
	if (frames.is_open()) {
		...
		while (frames >> frame) {
			if (i == 0) {
				i_curr_steps += 1;
				FrameIndex += 1;
				//printf("%d%d", i_curr_steps, FrameIndex);
				KeyFrame[FrameIndex-1].movAvion_x = std::stof(frame);
				printf("movAvion_x = %f", KeyFrame[FrameIndex - 1].movAvion_x);
				i++;
			}
			else if (i == 1) {
				KeyFrame[FrameIndex - 1].movAvion_y = std::stof(frame);
				printf("movAvion_y = %f", KeyFrame[FrameIndex - 1].movAvion_y);
				i++;
			}
			else if (i == 2) {
				KeyFrame[FrameIndex - 1].giroAvion = std::stof(frame);
				printf("giroAvion = %f", KeyFrame[FrameIndex - 1].giroAvion);
				i++;
			}
			else if (i == 3) {
				KeyFrame[FrameIndex - 1].giroAlaIzq = std::stof(frame);
				printf("giroAlaIzq = %f", KeyFrame[FrameIndex - 1].giroAlaIzq);
				i++;
			}
			else if (i == 4) {
				KeyFrame[FrameIndex - 1].giroAlaDer = std::stof(frame);
				printf("giroAlaDer = %f", KeyFrame[FrameIndex - 1].giroAlaDer);
				i = 0;
			}
		}
		frames.close();
